...
if video_url:
    ...
    if video_id:
        ...
        if comments:
            ...
            # 调用绘图函数，传入分析结果
            aspect_sentiment_scores = analyze_aspects_with_absa(cleaned_comments)  # 假设该函数已经定义
            plot_aspect_distribution(aspect_sentiment_scores)

            # 定义计算每个方面的评论数量和情感分数的函数
            def count_aspects_and_scores(comments, sentiment_scores):
                ...
                for comment, score in zip(comments, sentiment_scores):
                    aspects = classify_comment_aspect(comment.lower())  # 假设该函数已经定义
                    for aspect in aspects:
            ...
            # 使用 Streamlit 在界面上显示 emoji 使用百分比
